    ...
    public partial class voucher : System.Web.UI.Page
    {
        ...
        protected void BtnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                this.AdvancePayment = float.Parse(AdvancePay.Text);
                this.Tax = float.Parse(txtTax.Text);
                this.GrossAmount = float.Parse(GrossAmt.Text);
                this.NetAmount = (GrossAmount - (AdvancePayment + Tax));

                SqlConnection cnn = new SqlConnection(sqlcon);
                cnn.Open();
                String add = "Insert into asst(voucherId,billId,date,supplierName,vehicalNumber,material,washStatus,acceptedBag,netWeight,paymentMethod,chequeNumber,rate,netAmount,paidStatus,loadStatus,custNetWeight)values(@voucherId,@billId,@date,@supplierName,@vehicalNumber,@material,@washStatus,@acceptedBag,@netWeight,@paymentMethod,@chequeNumber,@rate,@netAmount,@paidStatus,@loadStatus,@custNetWeight)";
